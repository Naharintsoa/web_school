...
router.get('/', async (req, res) => {
  try {
    ...
    if (year) {
      params.push(year);
      conditions.push(`(school_year = $${params.length} OR school_year IS NULL)`);
    }
    if (school) {
      params.push(school);
      conditions.push(`(school = $${params.length} OR school IS NULL)`);
    }
